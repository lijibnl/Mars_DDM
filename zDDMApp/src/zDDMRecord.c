...
static long init_record(zDDMRecord *pscal, int pass)
{
    ...
    globalstr.globalstr_int = 0;
    channelstr.channelstr_int = 0;
    ...
    if (pass == 0)
    {
        ...
        if (pscal->nelm < 32)
            pscal->nelm = 32; /* Minimum system is 1 chip, 32 channels */
        if (nchips <= 0)
            nchips = 1;
        ...
        for (i = 0; i < pscal->nchips; i++)
        {
            ...
            globalstr[i].globalstr_bit.pa = 380;
            thrsh[i] = globalstr[i].globalstr_bit.pa;
            globalstr[i].globalstr_bit.pb = 102;
            pscal->tpamp = globalstr[i].globalstr_bit.pb;
            globalstr[i].globalstr_bit.sbn = 1;
            globalstr[i].globalstr_bit.sb = 1;
            globalstr[i].globalstr_bit.rm = 1;
            globalstr[i].globalstr_bit.senfl1 = 0;
            globalstr[i].globalstr_bit.senfl2 = 1;
            globalstr[i].globalstr_bit.sbm = 1;
            globalstr[i].globalstr_bit.sl = 0; /* make 2pA default */
            globalstr[i].globalstr_bit.slh = 0;

            printf("Aux buffer[%i]=%i\n", i, globalstr[i].globalstr_bit.sbm);
            globalstr[i].globalstr_bit.saux = 0;
            globalstr[i].globalstr_bit.sp = 1;
            pscal->pol = globalstr[i].globalstr_bit.sp;
        }
        for (i = 0; i < pscal->nelm; i++)
        {
            channelstr[i].channelstr_bit.sm = 0;
            chen[i] = 0;
            channelstr[i].channelstr_bit.st = 0;
            tsen[i] = 0;
            channelstr[i].channelstr_bit.sel = 1;
            pscal->loao = channelstr[i].channelstr_bit.sel;
            channelstr[i].channelstr_bit.da = 3; /* 3-bits, mid-scale*/
            thtr[i] = channelstr[i].channelstr_bit.da;
            channelstr[i].channelstr_bit.dp = 7; /* 4-bits, mid-scale*/
            putr[i] = channelstr[i].channelstr_bit.dp;
            slp[i] = 1.0;
            offs[i] = 0.0;
            intens[i] = 0;
        }
...
static long process(zDDMRecord *pscal)
{
    ...
    if (pscal->us == USER_STATE_IDLE && pscal->cont && pscal->ss != zDDM_STATE_COUNTING)
    {
        ...
        if (justFinishedUserCount)
            dly_sec = MAX(pscal->dly1, zDDM_wait_time);
        if (putNotifyOperation)
            dly_sec = MAX(pscal->dly1, zDDM_wait_time);
        ...
        if (dly == 0.0 || pscal->cnt == 0)
        {
            ...
            else
            {
                ...
                case USER_STATE_WAITING:
                    ...
                    if (pdelayCallback->timer)
                        epicsTimerCancel(pdelayCallback->timer);
                    pscal->us = USER_STATE_IDLE;
                    break;
