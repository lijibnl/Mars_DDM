...
EPICSTHREADFUNC event_publish(struct zDDMRecord *psr)
{
    ...
    while (1)
    {
        ...
        else if ((prevframestat == 1) && (framestat == 0))
        {
...
{ /* stuff structs into chp */
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        {
        case 0:
            pl_register_write(fd, MARS_CONF_LOAD, 0x00010000);
            // fpgabase[MARS_CONF_LOAD]=0x00010000;
            break;
        case 1:
            pl_register_write(fd, MARS_CONF_LOAD, 0x00020000);
            // fpgabase[MARS_CONF_LOAD]=0x00020000;
            break;
        case 2:
            pl_register_write(fd, MARS_CONF_LOAD, 0x00040000);
            // fpgabase[MARS_CONF_LOAD]=0x00040000;
            break;
        case 3:
            pl_register_write(fd, MARS_CONF_LOAD, 0x00080000);
            // fpgabase[MARS_CONF_LOAD]=0x00080000;
            break;
        case 4:
            pl_register_write(fd, MARS_CONF_LOAD, 0x00100000);
            // fpgabase[MARS_CONF_LOAD]=0x00100000;
            break;
        case 5:
            pl_register_write(fd, MARS_CONF_LOAD, 0x00200000);
            // fpgabase[MARS_CONF_LOAD]=0x00200000;
            break;
        case 6:
            pl_register_write(fd, MARS_CONF_LOAD, 0x00400000);
            // fpgabase[MARS_CONF_LOAD]=0x00400000;
            break;
        case 7:
            pl_register_write(fd, MARS_CONF_LOAD, 0x00800000);
            // fpgabase[MARS_CONF_LOAD]=0x00800000;
            break;
        case 8:
            pl_register_write(fd, MARS_CONF_LOAD, 0x01000000);
            // fpgabase[MARS_CONF_LOAD]=0x01000000;
            break;
        case 9:
            pl_register_write(fd, MARS_CONF_LOAD, 0x02000000);
            // fpgabase[MARS_CONF_LOAD]=0x02000000;
            break;
        case 10:
            pl_register_write(fd, MARS_CONF_LOAD, 0x04000000);
            // fpgabase[MARS_CONF_LOAD]=0x04000000;
            break;
        case 11:
            pl_register_write(fd, MARS_CONF_LOAD, 0x08000000);
...
int load_ad9252reg(int chipSel, int addr, int data)
{
    ...
    // address
    for (j = 12; j >= 0; j--)
        send_spi_bit(chipSel, addr >> j);

    // data
    for (j = 7; j >= 0; j--)
        send_spi_bit(chipSel, data >> j);
...
static const iocshArg *const devzDDMConfigArgs[3] = {
    &devzDDMConfigArg0,
    &devzDDMConfigArg1,
    &devzDDMConfigArg2,
};
...
{
    devzDDMConfig((int)args[0].ival, (unsigned short)args[1].ival, (int)args[2].ival);
}
...
static const iocshArg *const pokeArgs[2] = {
    &pokeArg0,
    &pokeArg1,
};
